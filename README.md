...
**An agent-to-agent trust marketplace, paid per call, settled on Algorand via x402.**
...
Getting a well-researched, trustworthy answer today means relying on either a single LLM that can hallucinate, or a human manually stitching together separate search, summarization, and fact-checking tools. There's no standard way for one AI agent to autonomously discover a specialist agent, pay it for a single use, and prove afterward exactly what was paid to whom for which piece of the work. Existing solutions rely on subscriptions or API keys, which don't fit a world where agents transact with each other in real time, on demand, without a human approving each call.

## The Solution

TrustMesh is an orchestrator agent paired with three specialist worker agents — **Provenance**, **Verification**, and **Trust Synthesis** — each running as its own micro-service, each gated by the x402 payment protocol. When a user asks a research or verification question, the orchestrator calls each worker in sequence, paying per call in real USDC the moment it uses that worker's endpoint. Payment settles instantly on Algorand testnet, and the orchestrator assembles the verified results into a final cited report, alongside a transparent, on-chain audit trail of every micropayment made.

Every request produces:
- A synthesized trust report (Gemini-generated)
- A verified claim with a confidence score
- A payment audit trail linking to real settled transactions on Algorand

## Live Demo

Open `index.html` served via local server (with the orchestrator and all three agent servers running locally), type a verification question, and click **Verify**. Within seconds you'll see:

1. The orchestrator paying the Provenance Agent ($0.01 USDC) for live web search and source tracking
2. The orchestrator paying the Verification Agent ($0.005 USDC) to verify a claim
3. The orchestrator paying the Trust Synthesis Agent ($0.005 USDC) to condense and compile the findings
...
User → Orchestrator (Express server, port 4020)
          │
          ├─ Provenance Agent (port 4021)       — x402-gated, real Tavily search (POST /provenance)
          ├─ Verification Agent (port 4023)     — x402-gated, Gemini-verified claims (POST /verify)
          └─ Trust Synthesis Agent (port 4022)   — x402-gated, Gemini-condensed summary (POST /synthesize)
          │
...
1. Orchestrator calls a worker's endpoint (e.g. `POST /provenance`).
...
- **Search:** Tavily API
- **LLM:** Google Gemini API — model: `gemini-3.1-flash-lite` (synthesis + verification + report compilation)
...
|---|---|---|---|
| Provenance Agent | `POST /provenance` | $0.01 USDC | Live web search via Tavily |
| Verification Agent | `POST /verify` | $0.005 USDC | Verifies a claim via Gemini, returns verdict + confidence |
| Trust Synthesis Agent | `POST /synthesize` | $0.005 USDC | Condenses findings via Gemini |
...
cd workers/provenance-agent && npm install
cd ../trust-synthesis-agent && npm install
cd ../verification-agent && npm install
...
cd workers/provenance-agent && npm run dev        # :4021
cd workers/trust-synthesis-agent && npm run dev    # :4022
cd workers/verification-agent && npm run dev      # :4023
